...
public:
    ...
    Grid(string mapPath, float pixelToMeter = 1.0, float raduis = 0.1, int costSize = 5) {
        ...
        cout << "Map Size: " << map.cols * pixel2meter << " mtr," << map.rows * pixel2meter << " mtr\n";
        if (map.empty())
        {
            cout << "[ERROR] the was no map loaded...";
            exit;
        }
    ...
    vector<vector < Node >> ConvertMatToVector(cv::Mat map) {
